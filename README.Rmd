...
-   **Species impact indicator** with `compute_species_indicator()`:
...
`compute_regional_indicator()` to compute impact indicators of alien taxa,
...
`b3gbi::process_cube()` and (2)
Environmental Impact Classification for Alien Taxa (EICAT) data
...
We compute the impact indicator per species by combining the impact score and occurrence of individual species.
...
impact scores of each site of the study region. The impact indicator use one of the methods named above in the
impact per site. The `ci_type` argument allows the function
`compute_regional_indicator()` to calculate the confidence interval of the
regional impact indicator using a bootstrapping method via the
...
extent to which the regional indicator is driven by individual species by
...
particular species. We use the `cross_validation_cube()` from **dubicube**
...
boot_fun <- function(data){
  impact_values <- compute_regional_indicator(
    cube = data,
    impact_data = eicat_acacia,
    method = "mean_cum",
    ci_type = "none"
...
```{r RMSE_sensitivity, echo=FALSE}
# perform leave-one-species-out cross-validation
cv_results <- dubicube::cross_validate_cube(
...
```{r Error_sensitivity, echo=FALSE}
